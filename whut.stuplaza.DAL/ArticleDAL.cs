...
namespace whut.stuplaza.DAL
{
    public class ArticleDAL
    {
        ...
        {
            string sql = "select COUNT(*) from T_stuplazaArticle";
        ...
        /// <returns></returns>
        public int Insert(T_stuplazaArticle model)
        {
            ...
            string sql = "insert into T_stuplazaArticle values(@id,@title,@sector,@category,@topic,@content,@poststaff,@annexaddr,@time,@column)";
        ...
        public bool Update(T_stuplazaArticle model)
        {
            ...
            strSql.Append("update T_stuplazaArticle set ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from T_stuplazaArticle ");
            strSql.Append("where C_ArticleId=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Char,11)
			};
        ...
        //public List<T_stuplazaArticle> GetArticleList(int i)
        //{
        ...
        /// <returns></returns>
        public T_stuplazaArticle DataRowToModel(DataRow row)
        {
        ...
        {
            string sql = "select top " + total + " * from T_stuplazaArticle where C_ArticleCategory=" + category + "order by C_ArticleTime desc";
        ...
        public T_stuplazaArticle GetArticleById(string id)
        {
            ...
            string sql = "select * from T_stuplazaArticle where C_ArticleId=@id";
            T_stuplazaArticle model = new T_stuplazaArticle();
        ...
        public DataTable GetIdByTime(string dateTime)
        {
            ...
            string sql = string.Format("select top(1) *from  T_stuplazaArticle where(C_ArticleId like '{0}%') order by C_ArticleId desc", dateTime);
        ...
        {
            string sql = "select * from T_stuplazaArticle where C_ArticleColumn=@column order by C_ArticleTime desc";
            SqlParameter[] pms = { new SqlParameter("@column", columnid) };
            return SqlHelper.ExecuteDataTable(sql, CommandType.Text, pms);
        }
        //获取文章内容中有图片标签的list
        public DataTable GetListByContent()
        {
            string sql = "select top 4 * from T_stuplazaArticle where C_ArticleContent like '%<img%/>%' order by C_ArticleTime desc";
            return SqlHelper.ExecuteDataTable(sql, CommandType.Text);
        }
        ...
        {
            string sql = "select top " + beginIndex + " * from T_stuplazaArticle where C_ArticleId not in (select top " + endIndex + " C_ArticleId from T_stuplazaArticle where C_ArticleCategory=" + cateGory + " order by C_ArticleTime desc) and C_ArticleCategory=" + cateGory + " order by C_ArticleTime desc";
            return SqlHelper.ExecuteDataTable(sql, CommandType.Text);
        }
        //根据存储过程来查询专题的文章集合
